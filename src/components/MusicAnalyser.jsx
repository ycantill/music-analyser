...
import { useGuitarSound } from '../core/useGuitarSound.js';
import { detectChord } from '../core/chords.js';
...
export default function MusicAnalyser() {
  ...
  const handleClickNote = useCallback(
    (noteClicked) => {
      const newChord = [{ name: noteClicked.name, octave: noteClicked.octave }];
  ...
  const handleSetScale = useCallback((noteClicked) => {
    const newChord = [...chordRef.current, { name: noteClicked.name, octave: noteClicked.octave }];
    chordRef.current = newChord;
    setChord(newChord);
    playChord(newChord);
    setNotes((prev) =>
  ...
  const handleRemoveScale = useCallback((noteClicked) => {
    const newChord = chordRef.current.filter((n) => n.name !== noteClicked.name);
    chordRef.current = newChord;
    setChord(newChord);
    playChord(newChord);
    setNotes((prev) =>
  ...
  const chordName = useMemo(() => {
    ...
    const rootNote = notes.find(
      (n) => n.tone && chord.some((c) => c.name === n.name && c.octave === n.octave)
    );
    if (!rootNote) return null;
    const semitones = chord.map((c) => {
      const note = notes.find((n) => n.name === c.name && n.octave === c.octave);
      return note?.interval?.semitones ?? 0;
    });
    return detectChord(rootNote.name, semitones);
  }, [chord, notes]);

  const hasNotes = notes.some((n) => n.tone);
      ...
      {!hasNotes ? (
        <div className="app-header-wrap">
          <a
            ...
            href="https://github.com/ycantill/music-analyser"
            target="_blank"
          ...
          <header className="fretboard-header">
            ...
            <p className="subtitle-default">Toca alguna cuerda y explora</p>
            <p className="subtitle-scroll">Arrastra el diapasón, toca alguna cuerda y explora</p>
          </header>
        </div>
      ) : null}
      <GuitarInstrument

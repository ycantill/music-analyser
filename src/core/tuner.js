...
const getNotesNamesIndexDistance = (start, end) =>
  NOTES_NAMES.indexOf(start) - NOTES_NAMES.indexOf(end);
...
const tune = (start = 'C3', octaves = 1) => {
  ...
  const semitonesFromCNoteToStartNote = Math.abs(
    getNotesNamesIndexDistance(DEFAULT_FIRST_NOTE, allNotesNames[allNotesNames.length - 1])
  );

  const notes = new Map(
    allNotesNames.reduce((accumulator, name, semitones) => {
